...
program
	.name( "portfolio-gallery-utils" )
	.description( "CLI tools for managing the portfolio gallery" )
	.version( "1.0.0" );

program
	.command( "convert" )
	.description( "Convert unprocessed portfolio files" )
	.action( async () => {
...
program
	.command( "html" )
	.description( "Generate html from portfolio files" )
	.action( async () => {
		const { dstFolder } = getWorkingFolderPaths();
		const output = await gallery.getFlexHtml( dstFolder, "/content/images/portfolio", "portfolio" );
		console.log( output );
	} );
...
function getWorkingFolderPaths() {
	const dstFolder = path.join( __dirname, "..", "site", "content", "images", "portfolio" );
